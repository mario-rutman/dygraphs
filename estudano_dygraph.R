...
ts1 <- ts(1:10, frequency = 4, start = c(1959, 2)) # trimestral
ts1
ts2 <- ts(1:12, frequency = 4, start = c(1959, 1))
ts2
ts3 <- ts(1:36, frequency = 12, start = c(1959, 1)) # mensal
ts3
ts4 <- ts(1:104, frequency = 52, start = c(1959, 1)) # semanal
ts4

# Ao invés de cria um df com coluna marcando o tempo.
# Vou criar uma time-series, uma ts, em que uma linha será a arrecadação
# e a outra a GIA

# criando a GIA
set.seed(416)
gia <- runif(48, min = 8000, max = 200000)

#criando a arrecadação: arrec
set.seed(127)
arrec <- runif(48, min = 0, max = 200000)

# criando o data frame com a gia e a arrecadação.
df_gia_arrec <- data_frame(gia, arrec) %>% 
# forçando que a arrec seja menor que a gia
mutate(arrec = ifelse(arrec > gia, gia-100, arrec)) %>% 
# colocando no formato de milhares de reais
mutate(arrec = round(arrec/1000, 0)) %>% 
mutate(gia = round(gia/1000, 0))  


# Criando o ts, que é mensal e começa em jan-2014.
ts_gia_arrec <- ts(df_gia_arrec, frequency = 12, start = c(2014, 1))

# fazendo o gráfico com dygraphs
dygraph(ts_gia_arrec) %>% 
  dySeries("gia", label = "GIA") %>% # Mudando os labels.
  dySeries("arrec", label = "Arrecadação") %>% # Mudando os labels.
  # 'Empilhando' as linhas, escolhendo as cores e opacidade.
  dyOptions(stackedGraph = TRUE, fillGraph = TRUE, 
            colors = c('red', 'green'), fillAlpha = 0.8, drawGrid = FALSE,
            ) %>% # 'Empilhando' as linhas e escolhendo as cores.
  # o dyOptions é muito extenso olhar no 
  # https://cran.r-project.org/web/packages/dygraphs/dygraphs.pdf
  dyRangeSelector(height = 20) %>%  # Altura do range selector.
  dyLegend(show = "follow") 

# Agora com gráfico de barras.
dygraph(ts_gia_arrec) %>% dyBarChart() %>% 
  dySeries("gia", label = "GIA") %>% # Mudando os labels.
  dySeries("arrec", label = "Arrecadação") %>% # Mudando os labels.
  # 'Empilhando' as linhas, escolhendo as cores e opacidade.
  dyOptions(stackedGraph = FALSE, fillGraph = TRUE, 
  colors = c('red', 'green'), fillAlpha = 0.8, drawGrid = FALSE) %>% # 'Empilhando' as linhas e escolhendo as cores.
  # o dyOptions é muito extenso olhar no 
  # https://cran.r-project.org/web/packages/dygraphs/dygraphs.pdf
  dyRangeSelector(height = 20) %>%  # Altura do range selector.
  dyLegend(show = "follow")
